  ...
  async start() {
    try {
      ...
      const [role, appData, metaData] = await Promise.all([
        this._context.role ? this._socketSend('set_socket_role', this._context.role) : undefined,
...
ReacoloDirtyModelSync.DISCONNECTED = Symbol('disconnected');
ReacoloDirtyModelSync.DATA_UPDATE = Symbol('data:update');
ReacoloDirtyModelSync.CONTEXT_UPDATE = Symbol('context:update');
ReacoloDirtyModelSync.CONNECTION_ERROR = Symbol('connection:error');
