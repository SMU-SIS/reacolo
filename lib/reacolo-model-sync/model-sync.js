...
export default class ReacoloModelSync extends EventEmitter {
  ...
  constructor(serverAddress = DEFAULT_SERVER_ADDR, role) {
    super();
  ...
  async setRole(role) {
    await this._socket.sendRequest(MessageTypes.SET_SOCKET_ROLE_MSG_TYPE, role);
    ...
    try {
      ...
      const [role, appData, metaData] = await Promise.all([
        ...
        this._context.role == null ? undefined : (
          this._socket.sendRequest(MessageTypes.SET_SOCKET_ROLE_MSG_TYPE, this._context.role)
        ),
