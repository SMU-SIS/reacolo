...
describe('bindContext', () => {
  ...
  it(
    ...
    () => {
      const Context = bindContext();
      expect(Context.getTargets(<Context foo="foo" bar="bar" />)).toEqual({
        foo: 'foo',
        bar: 'bar',
      });
      expect(
        Context.getTargets(
          <Context foo="foo" bar="bar" default>
            <div />
          </Context>,
        ),
  ...
  it('properly uses propsToTargets when provided as an object to extends its targets', () => {
    ...
    expect(Context.getTargets(contextElement)).toEqual({
      bar: 'bar',
      foo: 'foo',
    });
  });
  it('gives the priority on its own properties rather than propsToTargets', () => {
    const Context = bindContext({ foo: 'foo' });
    const contextElement = (
      <Context bar="bar" foo="stuff" default>
        <div />
      </Context>
    );
    expect(Context.getTargets(contextElement)).toEqual({
